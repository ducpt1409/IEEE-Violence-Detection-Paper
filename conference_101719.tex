...
Beyond aggregate metrics, it is instructive to examine what the pipeline actually produces on individual clips. Fig.~\ref{fig:output_examples} presents three representative inference results drawn from the test partition, each annotated with the system's final prediction and the detection bounding boxes assigned by Stage~A.
...
The four cases jointly illustrate how the cascade discriminates across interaction complexity levels. In scenario~(a), two pedestrians walk independently: the inter-person distance exceeds $T_{adaptive}$ for their respective bounding box scales, each is assigned a singleton track, and Stage~A performs no kinematic analysis. Scenario~(b) is the canonical two-person violent event---the abrupt body displacement during the strike generates an $A_{max}$ spike above the physics threshold, causing Stage~A to form an interaction group and issue a preliminary alert; Stage~B then processes the 16-frame ROI clip and confirms the label at confidence 0.88. Scenario~(c) scales the problem to four participants: adaptive grouping merges all of them into a single ROI because every pairwise distance falls within $T_{adaptive}$, and the resulting kinematic chaos produces a confident group-level violence prediction. Scenario~(d) is the hardest true negative: five pedestrians form three proximate groups, yet within each group the motion is slow and directionally coherent, keeping $A_{max}$ and $\sigma_{heat}$ below both physics thresholds and correctly suppressing the Stage~A alert. The 40 missed detections (FN) in the full test set are predominantly slow-escalation exchanges that look kinematically similar to scenario~(d) until the decisive impact frame arrives.

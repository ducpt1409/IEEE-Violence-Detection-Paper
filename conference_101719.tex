...
\noindent where each $\text{head}_i = \text{TMHA}(H^{\ell-1}W_{Q_i},\, H^{\ell-1}W_{K_i},\, H^{\ell-1}W_{V_i})$ and $W_O \in \mathbb{R}^{h d_k \times d_0}$ is a learnable output projection. The score matrix $QK^{\top} \in \mathbb{R}^{T \times T}$ assigns a scalar weight to every ordered frame pair $(t_i, t_j)$, directly connecting any two positions in the clip.

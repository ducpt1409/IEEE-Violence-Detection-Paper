...
The flow estimate produces a motion vector $\bigl(u(x, y),\; v(x, y)\bigr)$ at every pixel $(x, y)$, where $u(x, y)$ is the horizontal displacement (in pixels) of the image content at pixel $(x, y)$ between frame $t-1$ and frame $t$, and $v(x, y)$ is the corresponding vertical displacement. From this vector field we build a single-channel "heatmap" by taking the per-pixel magnitude:
...
\noindent $M(x, y)$ has the same spatial dimensions as the input frame and acts as a per-pixel motion-energy map: pixels where things are moving fast (punches, kicks, swinging limbs) have large $M$, while pixels with static content (walls, parked objects) have $M \approx 0$. Inside the ROI this gives us a localized motion picture that the box centroid alone cannot produce. Optical flow has a long history as a complementary motion cue in both action recognition~\cite{simonyan_two-stream_2014} and violence detection~\cite{gracia_fast_2015}, and we use it here in the same spirit.
...
\begin{equation}
\begin{aligned}
\mu_v &= \tfrac{1}{|\mathcal{S}|}\!\!\sum_{(p,\tau)\in\mathcal{S}}\!\! v_\tau^{(p)}, \\
\sigma_v &= \sqrt{\tfrac{1}{|\mathcal{S}|}\!\!\sum_{(p,\tau)}\!\! \bigl(v_\tau^{(p)}-\mu_v\bigr)^{2}}, \\
\mu_a &= \tfrac{1}{|\mathcal{S}|}\!\!\sum_{(p,\tau)}\!\! a_\tau^{(p)}, \quad
A_{\max} = \!\!\max_{(p,\tau)\in\mathcal{S}}\!\! a_\tau^{(p)}, \\
S_{\mathrm{interaction}} &= \mathrm{std}_{\mathrm{circ}}\!\bigl(\{\theta_\tau^{(p)}\}_{(p,\tau)\in\mathcal{S}}\bigr), \\
\mu_{\mathrm{heat}} &= \tfrac{1}{|\mathcal{R}|}\!\!\sum_{(x,y)\in\mathcal{R}}\!\! M(x,y), \\
\sigma_{\mathrm{heat}} &= \sqrt{\tfrac{1}{|\mathcal{R}|}\!\!\sum_{(x,y)}\!\! \bigl(M(x,y)-\mu_{\mathrm{heat}}\bigr)^{2}},
\end{aligned}
\label{eq:aggregation}
\end{equation}

\noindent where $v_\tau^{(p)}$, $a_\tau^{(p)}$, and $\theta_\tau^{(p)}$ are the per-frame quantities defined in Eq.~\ref{eq:velocity} and Eq.~\ref{eq:accel_angle} for person $p$ at frame $\tau$; $M(x, y)$ is the optical-flow magnitude from Eq.~\ref{eq:heatmap_magnitude}; and $\mathrm{std}_{\mathrm{circ}}(\cdot)$ denotes the circular standard deviation of an angle set. We use the circular variant because $\theta \in (-\pi, \pi]$ is cyclic: a linear standard deviation would label two nearly identical angles like $-179^\circ$ and $179^\circ$ as $358^\circ$ apart, when in fact they are only $2^\circ$ apart.

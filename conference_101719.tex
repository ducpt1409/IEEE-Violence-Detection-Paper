...
Within each stage, features are refined by a pair of consecutive Transformer blocks. The first block applies W-MSA; the second applies SW-MSA over windows shifted by half the window size to enable cross-window communication:
\begin{align}
\hat{z}^{l}   &= \text{W-MSA}\!\left(\text{LN}(z^{l-1})\right) + z^{l-1}, \label{eq:wmsa_pre} \\
z^{l}         &= \text{MLP}\!\left(\text{LN}(\hat{z}^{l})\right) + \hat{z}^{l}, \label{eq:wmsa_post} \\
\hat{z}^{l+1} &= \text{SW-MSA}\!\left(\text{LN}(z^{l})\right) + z^{l}, \label{eq:swmsa_pre} \\
z^{l+1}       &= \text{MLP}\!\left(\text{LN}(\hat{z}^{l+1})\right) + \hat{z}^{l+1}. \label{eq:swmsa_post}
\end{align}

\noindent In Eqs.~\ref{eq:wmsa_pre}--\ref{eq:swmsa_post}, $z^{l-1}$ is the input token sequence to the $l$-th block, LN denotes layer normalization, and MLP is a two-layer feed-forward network with GELU activation. The residual connections in each block stabilize gradient flow across the four-stage hierarchy.

Within each MSA block the attention is computed as:
\begin{equation}
\text{Attention}(Q, K, V) = \text{SoftMax}\!\left(\frac{QK^{\top}}{\sqrt{d_k}} + B\right)V,
\label{eq:swin_attention}
\end{equation}

\noindent where $Q, K, V \in \mathbb{R}^{N \times d_k}$ are the query, key, and value matrices ($N$ is the number of tokens in the window and $d_k$ is the per-head dimension), and $B \in \mathbb{R}^{N \times N}$ is a learnable relative position bias matrix indexed by the relative spatial offset between each token pair. For Swin~Tiny with window size $M = 7$, each window contains $N = M^2 = 49$ patch tokens, so $B \in \mathbb{R}^{49 \times 49}$. $B$ encodes the relative spatial offset between every pair of patch positions, so the attention picks up local spatial structure inside each window without needing absolute positional encodings.
...
\noindent The input $H^0$ is then processed by $L$ successive Transformer encoder layers, each comprising Temporal Multi-Head Attention (TMHA) over the temporal axis followed by a position-wise feed-forward sublayer and residual connections. Within each layer $\ell$, the token sequence $H^{\ell-1} \in \mathbb{R}^{T \times d_0}$ is first projected into query, key, and value matrices:
\begin{align}
Q &= H^{\ell-1}W_Q, \label{eq:proj_q} \\
K &= H^{\ell-1}W_K, \label{eq:proj_k} \\
V &= H^{\ell-1}W_V, \label{eq:proj_v}
\end{align}

\noindent where $W_Q, W_K, W_V \in \mathbb{R}^{d_0 \times d_k}$ are learnable projection matrices and $d_k = d_0 / h = 96$ is the per-head dimension. The resulting $Q, K, V \in \mathbb{R}^{T \times d_k}$ are then passed to a single attention head. With $h = 8$ parallel heads, the outputs are concatenated and projected back:

\begin{equation}
\text{TMHA}(Q, K, V) = \text{SoftMax}\!\left(\frac{QK^{\top}}{\sqrt{d_k}}\right)V
\label{eq:tmha}
\end{equation}

\begin{equation}
\text{MultiHead}(H^{\ell-1}) = \text{Concat}(\text{head}_1, \ldots, \text{head}_h)\,W_O
\label{eq:multihead}
\end{equation}

\noindent where each head $i$ has its own learnable projections $W_{Q_i}, W_{K_i}, W_{V_i} \in \mathbb{R}^{d_0 \times d_k}$, and $W_O \in \mathbb{R}^{h d_k \times d_0}$ is a learnable output projection that combines all $h$ head outputs back into the $d_0$-dimensional embedding space. The score matrix $QK^{\top} \in \mathbb{R}^{T \times T}$ assigns a scalar weight to every ordered frame pair $(t_i, t_j)$, directly connecting any two positions in the clip.

Each encoder layer completes the forward pass with layer normalization and residual connections around both the attention and feed-forward sublayers:
\begin{align}
\hat{H}^{\ell} &= \text{MultiHead}(\text{LN}(H^{\ell-1})) + H^{\ell-1}, \label{eq:temp_attn_res} \\
H^{\ell}       &= \text{FFN}(\text{LN}(\hat{H}^{\ell})) + \hat{H}^{\ell}, \label{eq:temp_ffn_res}
\end{align}

\noindent where FFN is a two-layer feed-forward network with hidden dimension $4d_0 = 3072$ and GELU activation, applied position-wise along the temporal axis. In our configuration, $L = 2$ such layers are stacked, each with $h = 8$ attention heads (head dimension $d_k = 96$). A dropout rate of $p = 0.1$ is applied to both attention weights and FFN activations within each layer to mitigate overfitting on the relatively compact ROI clip dataset.
...
Aggregate metrics tell us how the system performs on average; per-clip outputs tell us how it actually behaves on individual cases. Fig.~\ref{fig:output_examples} shows four representative inference results from the test partition, each annotated with the system's final prediction and the detection bounding boxes assigned by Stage~A.

\begin{figure}[!htbp]
    \centering
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{image/case_non_vio.jpg}
        \caption{\textcolor{green}{\textbf{Non-Violent}} (pair): two pedestrians assigned to independent singleton tracks; inter-person distance exceeds $T_{adaptive}$, so no interaction group is formed and Stage~A is not triggered.}
        \label{fig:out_nv}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{image/case_vio.jpg}
        \caption{\textcolor{red}{\textbf{Violent}} (pair): bilateral altercation flagged as \textit{Group~1\,--\,Violence} (conf.$= 0.88$). Red ROI box enclosing both fighters is forwarded to Stage~B for deep confirmation.}
        \label{fig:out_v2}
    \end{subfigure}
    ...
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{image/case_agroup_vio.jpg}
        \caption{\textcolor{red}{\textbf{Violent}} (group): four-person brawl unified into a single \textit{Group~1\,--\,Violence} ROI (det. conf.$= 0.72$--$0.88$). High $A_{max}$ and $S_{interaction}$ activate Stage~A.}
        \label{fig:out_vg}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\columnwidth}
        \centering
        \includegraphics[width=\linewidth]{image/case_group_non_vio.jpg}
        \caption{\textcolor{green}{\textbf{Non-Violent}} (group): five pedestrians split into three independent groups (Group~1--3, blue boxes). Cohesive motion within each group keeps all kinematic features below thresholds.}
        \label{fig:out_nvg}
    \end{subfigure}

    \caption{Representative pipeline outputs on four held-out test clips. Green/blue group boxes denote non-violent predictions; red boxes denote violence alerts produced by Stage~A. (a)~Pair walking independently: singleton tracks, Stage~A silent. (b)~Two-person fight: red ROI issued by Stage~A, violence confirmed by Temporal Transformer (conf.$= 0.88$). (c)~Four-person brawl: all participants merged into one violent ROI with per-person detection confidence. (d)~Non-violent group traversal: five pedestrians correctly partitioned into three separate non-violent interaction groups.}
    \label{fig:output_examples}
\end{figure}

The four cases span the interaction-complexity range the cascade is designed to handle. In scenario~(a), two pedestrians walk past each other: the inter-person distance exceeds $T_{adaptive}$ for their respective bounding box scales, each is assigned a singleton track, and Stage~A performs no kinematic analysis. Scenario~(b) is a standard two-person fight. The abrupt body displacement during the strike pushes $A_{max}$ above the physics threshold, Stage~A forms an interaction group and issues a preliminary alert, and Stage~B then processes the 16-frame ROI clip and confirms the label at confidence 0.88. Scenario~(c) extends the problem to four participants: every pairwise distance falls within $T_{adaptive}$, so adaptive grouping merges all of them into a single ROI, and the elevated $A_{max}$ together with high $S_{interaction}$ produce a confident group-level violence prediction. Scenario~(d) is the trickiest true negative we observed: five pedestrians form three proximate groups, but within each group the motion is slow and directionally coherent, so both $A_{max}$ and $\mu_{heat}$ stay below the physics thresholds and Stage~A correctly stays silent. Most of the 40 missed detections (FN) in the full test set look kinematically similar to scenario~(d) until the decisive impact frame arrives, which is why slow-escalation footage remains the hardest failure mode of the system.
